...
public class MaristBaseRobot2024_Quad {
    ...
    public void driveFieldCentric(double gamepadXPow, double gamepadYPow, double gamepadRotPow, double DRIVE_SPEED){
        //Calculate target heading based off of gamepad input
        double gamepadTheta = Math.atan2(gamepadYPow, gamepadXPow);
        if(gamepadTheta < 0)gamepadTheta = 2*Math.PI +gamepadTheta;//angle wrapping to keep (-) angle values in a [0,2pi] range
        double mag = Math.sqrt((gamepadYPow*gamepadYPow) + (gamepadXPow*gamepadXPow));
        //calculate the heading, respective to the robot, that robot should move in
        double diffTheta = gamepadTheta - Math.toRadians(getOrientation());
        //Calculate motor powers
        double leftFrontPower = mag*Math.cos(diffTheta + (Math.PI/4))+gamepadRotPow;
        double leftRearPower = mag*Math.sin(diffTheta + (Math.PI/4))+gamepadRotPow;
        double rightFrontPower = mag*Math.sin(diffTheta + (Math.PI/4))-gamepadRotPow;
        double rightRearPower = mag*Math.cos(diffTheta + (Math.PI/4))-gamepadRotPow;
